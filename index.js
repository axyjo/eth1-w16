...
client.connect(25001, 'test-exch-dresden', function () {
...
var books = {};
var trades = {};
var qtys = {XLF: 0, BOND: 0, GS: 0, MS: 0, WFC: 0, VALE: 0, VALBZ: 0};
...
setInterval(function() {
  // What are people willing to buy 
  var stock = getBuyPrice("VALBZ");
  var etf = getBuyPrice("VALE");
 
  if (!stock || !etf) return;

  var difference = stock - etf;
  
  if (Math.abs(difference) <= 20) {
    console.log("NOT WORTH - VAL");
    return;

  }
  // positive if stock is bigger (basket's buy value is greater)
  if (difference > 0) {
      // sell-convert some ETFs to convert to the basket.
      convert("VALE", "SELL", 1);
      sendOrder("VALBZ", "BUY", stock, 1);
      console.log("convert to stock", difference)
  } else if (difference < 0) {
    // buy-convert some ETFs to convert to the ETF
      convert("VALE", "BUY", 1);
      sendOrder("VALBZ", "SELL", stock, 1);
      console.log("convert to etf", difference)
  }
}, 500);
